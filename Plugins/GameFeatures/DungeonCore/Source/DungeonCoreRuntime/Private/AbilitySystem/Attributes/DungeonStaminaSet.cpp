...
#include UE_INLINE_GENERATED_CPP_BY_NAME(DungeonStaminaSet)
...
void UDungeonStaminaSet::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
    Super::GetLifetimeReplicatedProps(OutLifetimeProps);

    DOREPLIFETIME_CONDITION_NOTIFY(UDungeonStaminaSet, Stamina, COND_None, REPNOTIFY_Always);
    DOREPLIFETIME_CONDITION_NOTIFY(UDungeonStaminaSet, MaxStamina, COND_None, REPNOTIFY_Always);
    DOREPLIFETIME_CONDITION_NOTIFY(UDungeonStaminaSet, BaseRegen, COND_OwnerOnly, REPNOTIFY_Always);

}

void UDungeonStaminaSet::OnRep_Stamina(const FGameplayAttributeData& OldValue)
{
    GAMEPLAYATTRIBUTE_REPNOTIFY(UDungeonStaminaSet, Stamina, OldValue);
...
void UDungeonStaminaSet::OnRep_MaxStamina(const FGameplayAttributeData& OldValue)
{
    GAMEPLAYATTRIBUTE_REPNOTIFY(UDungeonStaminaSet, MaxStamina, OldValue);
...
bool UDungeonStaminaSet::PreGameplayEffectExecute(FGameplayEffectModCallbackData& Data)
{
...
void UDungeonStaminaSet::OnRep_BaseRegen(const FGameplayAttributeData& OldValue)
{
    GAMEPLAYATTRIBUTE_REPNOTIFY(UDungeonStaminaSet, BaseRegen, OldValue);
}

void UDungeonStaminaSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
    ...
    if (Data.Target.HasMatchingGameplayTag(LyraGameplayTags::Cheat_GodMode) || Data.Target.HasMatchingGameplayTag(DungeonGameplayTags::Cheat_UnlimitedStamina))
    {
        MinimumStamina = 1.0f;
    }
    ...
    if (Data.EvaluatedData.Attribute == GetRegeningAttribute())
    {
        // Convert into +Stamina and then clamo
        SetStamina(FMath::Clamp(GetStamina() + GetRegening(), MinimumStamina, GetMaxStamina()));
        SetRegening(0.0f);
    }
...
void UDungeonStaminaSet::PreAttributeBaseChange(const FGameplayAttribute& Attribute, float& NewValue) const
{
...
void UDungeonStaminaSet::PreAttributeChange(const FGameplayAttribute& Attribute, float& NewValue)
{
...
void UDungeonStaminaSet::PostAttributeChange(const FGameplayAttribute& Attribute, float OldValue, float NewValue)
{
...
void UDungeonStaminaSet::ClampAttribute(const FGameplayAttribute& Attribute, float& NewValue) const
{
